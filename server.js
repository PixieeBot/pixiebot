...
config.port = process.env.PORT || config.port || 4961;
...
var files = fs.readdirSync(__dirname + '/static/leaderboard');
for (var i = 0; i < files.length; i++) {
	var item = files[i];
	var dest = "";
	if ((item.indexOf('.css') === item.length - 4) && (item.indexOf('.min.css') === -1)) {
		dest = item.substring(0, item.length - 4) + '.min.css';
		fs.writeFileSync(__dirname + '/static/leaderboard/' + dest, new cssCompressor().minify(fs.readFileSync(__dirname + '/static/leaderboard/' + item, 'utf8')).styles, 'utf8');
		...
		fs.writeFileSync(__dirname + '/static/leaderboard/' + dest, jsCompressor.minify(__dirname + '/static/leaderboard/' + item).code, 'utf8');
		logger.verbose('Compressed: ' + item + ' ==> ' + dest);
	}
}
...
app.use(express.static('static/leaderboard'));
app.get('/:id', function(req, res, next) {
	res.sendFile(__dirname + '/static/leaderboard/index.html');
});
